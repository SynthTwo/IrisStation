...
/datum/controller/subsystem/dynamic/proc/send_intercept()
	...
	. = "<b><i>Nanotrasen Department of Intelligence Threat Advisory, Cabriole Sector, TCD [time2text(world.realtime, "DDD, MMM DD")], [CURRENT_STATION_YEAR]:</i></b><hr>"
...
/datum/controller/subsystem/dynamic/proc/generate_advisory_level()
	...
	if(prob(PULSAR_REPORT_CHANCE))
		...
		advisory_string += "Your sector's advisory level is Pulsar Star. A large, unknown electromagnetic field has stormed through nearby surveillance equipment, causing major data loss. Partial data was recovered and showed no credible threats to Nanotrasen assets within the Cabriole Sector; however, the Department of Intelligence advises maintaining high alert against potential threats due to the lack of complete data."
		return advisory_string
	...
	if(round(shown_threat) == 0 && round(threat_level) == 0)
		...
		advisory_string += "Your sector's advisory level is White Dwarf. Our surveillance has ruled out any and all potential threats known in our database, eliminating most risks to our assets in the Cabriole Sector. We advise a lower level of security, alongside distributing resources on potential profit."
		return advisory_string
	...
	switch(round(shown_threat))
		if(0 to 19)
			...
			if (show_core_territory)
				...
				advisory_string += "Your sector's advisory level is Green Star. Surveillance information shows no credible threats to Nanotrasen assets within the Cabriole Sector at this time. As always, the Department of Intelligence advises maintaining vigilance against potential threats, regardless of a lack of known threats."
		if(20 to 39)
			advisory_string += "Advisory Level: <b>Yellow Star</b></center><BR>"
			advisory_string += "Your sector's advisory level is Yellow Star. Surveillance shows a credible risk of enemy attack against our assets in the Cabriole Sector. We advise a heightened level of security alongside maintaining vigilance against potential threats."
		if(40 to 65)
		...
		if(66 to 79)
			...
			advisory_string += "Your sector's advisory level is Red Star. The Department of Intelligence has decrypted Cybersun communications suggesting a high likelihood of attacks on Nanotrasen assets within the Cabriole Sector. Stations in the region are advised to remain highly vigilant for signs of enemy activity and to be on high alert."
		if(80 to 99)
